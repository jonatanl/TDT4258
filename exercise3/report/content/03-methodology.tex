...
Our game had two major resource drains, rendering the game on screen and calculating the game logic. In order to reduce expensive data operations we made sure to always use references to our data structures. We allocated some data to the data segment at compile time, and made sure that heap data was only allocated once and referred to in other parts of the program.
...
This way we could make sure that no expensive allocations or data moves were made in the middle of the game, increasing performance.

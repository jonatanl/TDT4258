...
So that we would not have to create the device file manually each time testing the driver, we rewrote the driver so that it would create the device file ``driver-gamepad'' on initialization, as described in section \ref{sec:creating-device-files}.

\subsection{Using the GPIO ports}
\todo{Write about the first implementation where the driver utilizes the GPIO ports}

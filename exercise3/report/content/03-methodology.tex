...
To develop our game we used an incremental approach, adding features whenever we had debugged our current features, and refactoring when nescessary. Our first versions of the game never outputted anything to the buffer at all, using only the serial connection to write status messages. We then implemented the basic draw system

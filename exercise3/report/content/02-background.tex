...
\subsubsection{Creating device files}
After device numbers have been allocated, a device file can be created manually by issuing the command:
\lstset{style=lststyle-terminal}
\begin{lstlisting}
mknod /dev/<device name> c <major rev> <minor rev>
\end{lstlisting}
Here \texttt{<major rev>} and \texttt{<minor rev>} should correspond to a pair of major and minor revision numbers owned by the driver.

\todo{Check if also <device name> needs to correspond with the name of the driver} 
...
As shown above, the function pointers in the \texttt{file\_operations} structure identify files using a \texttt{struct inode}. The \texttt{inode} plays a different role than the \texttt{file} structure: while the \texttt{file} structure stores information about an open file, the \texttt{inode} structure stores information about a file that is common among all its open instances. For a device file, it will also contain the following two important fields:
\begin{description}
